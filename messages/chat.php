...
function getlastActiveTime($username){
    $json3 = file_get_contents('includes/json/online-status.json');
...
function updatelastActiveTime(){
    ...
    file_put_contents('includes/json/online-status.json', $json);
}
        ...
        while($row=mysqli_fetch_array($sql_res))
        {
            ...
            <li class="person chatboxhead" id="chatbox1_<?php echo $username ?>" data-chat="person_<?php echo $id ?>" href="javascript:void(0);" onclick="javascript:chatWith('<?php echo $username ?>','<?php echo $id ?>','<?php echo $sesuserpic; ?>','<?php echo $onofst ?>')">
                <a href="javascript:void(0)">
                    <span class="userimage profile-picture min-profile-picture"><img src="<?php echo $picname; ?>" alt="<?php echo $username ?>" class="bg-theme"></span>
                    <span>
...
function savechat($con,$config){
    ...
    $Filecontent = "";
    $Mailcontent = "";
    $sql = "select * from `".$config['db']['pre']."messages` where ((to_uname = '".$_SESSION['username']."' AND from_uname = '".$_GET['uname']."' ) OR (to_uname = '".$_GET['uname']."' AND from_uname = '".$_SESSION['username']."' )) order by message_id DESC";
    $query = $con->query($sql);
    ...
    if($_GET['mail'] == "true"){
        $query1 = "SELECT email FROM `".$GLOBALS['MySQLi_user_table_name']."` where `".$GLOBALS['MySQLi_username_field']."` = '".$_SESSION['username']."' LIMIT 1";
        $result1 = $con->query($query1);
        $row1 = mysqli_fetch_assoc($result1);
...
{
    if($_GET['msgid'] == "last"){
        $query1 = "SELECT seen FROM `".$config['db']['pre']."messages` where to_uname = '".$_GET['uname']."' and from_uname = '".$_SESSION['username']."' ORDER BY message_id DESC LIMIT 1";
    }
    else{
        ...
        <div class="user-bg">
            <div class="overlay-box">
                <div class="user-content"> <a href="javascript:void(0)">
                        <img class="thumb-lg img-circle" src="<?php echo $picname; ?>" alt="<?php echo $username ?>"></a>
...
//    $query = "select $TFid,$TFusername,$TFname,$TFPicname from `".$GLOBALS['MySQLi_user_table_name']."` where `".$GLOBALS['MySQLi_userid_field']."` != '".$GLOBALS['sesId']."' ORDER BY id DESC";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sql = "SELECT 1 FROM `".$config['db']['pre']."messages` where to_uname = '".$_SESSION['username']."' AND from_uname = '$username' and seen = '0'";
        ...
        $queryU = "SELECT AD_id FROM `".$config['db']['pre']."messages` WHERE (to_uname = '".$_SESSION['username']."' AND from_uname = '{$username}') OR (to_uname = '".$username."' AND from_uname = '{$_SESSION['username']}')";
...
EOD;
    ...
    $get_uname2_result = mysqli_query($con,$get_uname2_query);
