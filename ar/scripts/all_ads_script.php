...
if($num_Ads <= 10){
    $ad_query = "SELECT * FROM `ADVERTISEMENT` WHERE `lang` = 'AR'  ORDER BY ". $suffix;
}else if($page == 0 || $page == 1){
    $ad_query = "SELECT * FROM `ADVERTISEMENT` WHERE `lang` = 'AR'  ORDER BY " . $suffix. " LIMIT 10";
}else if($page > 1){
    $start = (($page - 1) * 10);
    $ad_query = "SELECT * FROM `ADVERTISEMENT` WHERE `lang` = 'AR'  ORDER BY " . $suffix. " LIMIT 10 OFFSET $start";
}
