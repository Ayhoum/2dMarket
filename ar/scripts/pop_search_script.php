    ...
    while ($row = mysqli_fetch_assoc($run_query)){
        ...
        if(!empty($lon) && !empty($lat)){
            ...
            if($km <= $dis) {
                if($_GET['price'] != 'all'){
                    if($minPrice == 0){
                        $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'AR' && `title` LIKE '%{$tag}%' AND `price` <= '$maxPrice'";
                    }else if($maxPrice == 'max'){
                        $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'AR' && `title` LIKE '%{$tag}%' AND `price` >= '$minPrice'";
                    }else{
                        $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'AR' && `title` LIKE '%{$tag}%' AND (`price` >= '$minPrice' AND `price` <= '$maxPrice')";
                    }
                }else{
                    $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'AR' && `title` LIKE '%{$tag}%'";
                }
            ...
            <div class="col-md-6 col-lg-6 col-sm-6 col-xs-12 ">
                <div class="category-grid-box" style="background-color: white">
                    ...
                    <div class="category-grid-img">
                        ...
                        <!-- View Details --><a href="ad_page.php?ad_id=<?php echo $ad_id;?>" class="view-details">أنظر للتفاصيل</a>
                        <!-- Additional Info -->
        ...
        <div class="col-md-12 col-xs-12 col-sm-12 col-lg-12">
            <section class="advertising">
                <a href="all_product">
                    <div class="banner">
                        <div class="wrapper">
                            <span class="title">يبدو أنه لا يوجد أي إعلانات ضمن هذه الفئة؛ يرجى البحث في فئة أخرى!</span>
                        </div>
                    </div>
                    <!-- /.banner-->
                </a>
