...
if($_GET['price'] != 'all'){
    if($minPrice == 0){
        $ad_query_get_num = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          ...
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' AND `price` <= '$maxPrice' ORDER BY " . $suffix;
    }else if($maxPrice == 'max'){
        $ad_query_get_num = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' AND `price` >= '$minPrice' ORDER BY " . $suffix;
    }else{
        $ad_query_get_num = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN'  AND (`price` >= '$minPrice' AND `price` <= '$maxPrice') ORDER BY " . $suffix;
    }
}else{
    $ad_query_get_num = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' ORDER BY " . $suffix;
}
    ...
    if($_GET['price'] != 'all'){
        if($minPrice == 0){
            $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' AND `price` <= '$maxPrice' ORDER BY " . $suffix . " LIMIT 10";
        }else if($maxPrice == 'max'){
            $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' AND `price` >= '$minPrice' ORDER BY " . $suffix . " LIMIT 10";
        }else{
            $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN'  AND (`price` >= '$minPrice' AND `price` <= '$maxPrice') ORDER BY " . $suffix . " LIMIT 10";
        }
    }else{
        $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' ORDER BY " . $suffix . " LIMIT 10";
    }
    ...
    if($_GET['price'] != 'all'){
        if($minPrice == 0){
            $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' AND `price` <= '$maxPrice' ORDER BY " . $suffix . " LIMIT 10 OFFSET $start";
        }else if($maxPrice == 'max'){
            $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' AND `price` >= '$minPrice' ORDER BY " . $suffix . " LIMIT 10 OFFSET $start";
        }else{
            $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN'  AND (`price` >= '$minPrice' AND `price` <= '$maxPrice') ORDER BY " . $suffix . " LIMIT 10 OFFSET $start";
        }
    }else{
        $ad_query = "SELECT ADVERTISEMENT.*, ADDRESS.USER_id
                                          FROM ADVERTISEMENT
                                          LEFT JOIN ADDRESS ON ADVERTISEMENT.USER_id = ADDRESS.USER_id
                                          WHERE ADDRESS.country LIKE '%{$country}%' && ADVERTISEMENT.lang = 'EN' ORDER BY " . $suffix . " LIMIT 10 OFFSET $start";
    }
