...
<div class="posts-masonry">
    ...
    while ($row = mysqli_fetch_assoc($run_query)){
        ...
        if(!empty($lon) && !empty($lat)){
            ...
            if($km <= $dis) {
                if($_GET['price'] != 'all'){
                    if($minPrice == 0){
                        $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'NL' && `title` LIKE '%{$tag}%' AND `price` <= '$maxPrice'";
                    }else if($maxPrice == 'max'){
                        $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'NL' && `title` LIKE '%{$tag}%' AND `price` >= '$minPrice'";
                    }else{
                        $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'NL' && `title` LIKE '%{$tag}%' AND (`price` >= '$minPrice' AND `price` <= '$maxPrice')";
                    }
                }else{
                    $queryAD = "SELECT * FROM `ADVERTISEMENT` WHERE  `lang` = 'NL' && `title` LIKE '%{$tag}%'";
                }
    ...
    if ($num_Ads <= 10) {
        $ad_query = "SELECT * FROM `ADVERTISEMENT` WHERE `id` IN ('$ids') AND `lang` = 'NL'  ORDER BY `ad_type` DESC," . $suffix;
    } else if ($page == 0 || $page == 1) {
        $ad_query = "SELECT * FROM `ADVERTISEMENT` WHERE `id` IN ('$ids') AND `lang` = 'NL'  ORDER BY `ad_type` DESC," . $suffix ." LIMIT 10";
    } else if ($page > 1) {
        $start = (($page - 1) * 10);
        $ad_query = "SELECT * FROM `ADVERTISEMENT` WHERE `id` IN ('$ids') AND `lang` = 'NL'  ORDER BY `ad_type` DESC," . $suffix ." LIMIT 10 OFFSET $start";
    }
            ...
            <div class="col-md-6 col-lg-6 col-sm-6 col-xs-12 ">
                <div class="category-grid-box" style="background-color: white">
                    ...
                    <div class="category-grid-img">
                        ...
                        <!-- View Details --><a href="ad_page.php?ad_id=<?php echo $ad_id;?>" class="view-details">Bekijk Details</a>
                        <!-- Additional Info -->
        ...
        <div class="col-md-12 col-xs-12 col-sm-12 col-lg-12">
            <section class="advertising">
                <a href="all_product">
                    <div class="banner">
                        <div class="wrapper">
                            <span class="title">Oh nee! het lijkt erop dat er geen advertenties in deze categorie zijn; Probeer maar iets anders!</span>
                        </div>
                    </div>
                    <!-- /.banner-->
                </a>
    ...
    <div class="col-md-12 col-xs-12 col-sm-12">
        <section class="advertising">
            <a href="<?php if (isset($_SESSION['id'])){echo "new_advertisement.php";  } else { echo "login.php"; }?>">
                <div class="banner">
                    <div class="wrapper">
                        <span class="title">Wilt u dat uw eigendom hier wordt vermeld?</span>
                        <span class="submit">Verzend het nu! <i class="fa fa-plus-square"></i></span>
                    </div>
                </div>
                <!-- /.banner-->
            </a>
...
<div class="text-center margin-top-30 block-center">
    <ul class="pagination ">
        ...
        if ($page != 0 && $page != 1) {
            ?>
            <li><a href="pop_search.php?tag=<?php echo $category_id;?>&dis=<?php echo $_GET['dis']; ?>&price=<?php echo $_GET['price']; ?>&order=<?php echo $_GET['order']; ?>&page=<?php echo $page - 1; ?>"> <i class="fa fa-chevron-left"
        ...
        for ($i = 1; $i <= $num_Ads; $i++) {
            ...
            <li <?php if ($i == $page || ($i == 1 && $page == 0)) {
                echo 'class="active"';
            } ?>><a href="pop_search.php?tag=<?php echo $category_id;?>&dis=<?php echo $_GET['dis']; ?>&price=<?php echo $_GET['price']; ?>&order=<?php echo $_GET['order']; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>

        <?php } ?>
        <?php
        if ($page != $num_Ads) {
            ?>
            <li><a href="pop_search.php?tag=<?php echo $category_id;?>&dis=<?php echo $_GET['dis']; ?>&price=<?php echo $_GET['price']; ?>&order=<?php echo $_GET['order']; ?>&page=<?php echo $page + 1; ?>"> <i class="fa fa-chevron-right"
                                                                                                                                                                                                                  aria-hidden="true"></i></a></li>
            <?php
        }
        ?>
